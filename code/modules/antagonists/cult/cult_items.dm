...
/obj/item/shuttle_curse/attack_self(mob/living/user)
	...
	if(SSshuttle.emergency.mode == SHUTTLE_CALL)
		...
		if(MAX_SHUTTLE_CURSES-totalcurses <= 0)
			to_chat(user, "<span class='danger'>You sense that the emergency escape shuttle can no longer be cursed.</span>")
		else if(MAX_SHUTTLE_CURSES-totalcurses == 1)
			to_chat(user, "<span class='danger'>You sense that the emergency escape shuttle can only be cursed 1 more time.</span>")
		else
			to_chat(user, "<span class='danger'>You sense that the emergency escape shuttle can only be cursed [MAX_SHUTTLE_CURSES-totalcurses] more times.</span>")
		qdel(src)
